...
describe('a radio', () => {
  ...
  const { ui, changeStation, increaseVolume, decreaseVolume, muteVolume } = buildUi({ type: 'fake_ui' })
  ...
  context('controlling the volume', () => {
    ...
    it('un-mutes the volume, remembering the previous level', async () => {
      radio.setVolume(70)
      await radio.on()
      await muteVolume({ ui })
      await muteVolume({ ui })
      await assertVolumeIs({ volume: 70, player })
    })
  })
})
